...
In this file I am installing Rocky Linux as a virtual machine inside VMware Workstation. The purpose of this Rocky Linux system is to eventually join it to the Windows Active Directory domain and learn Linux authentication, PAM, Kerberos, and security hardening. Before joining it to the domain, I need a clean installation with proper virtual hardware settings. I also need to understand what each installation choice means and why I am choosing it.

---

## Understanding What Rocky Linux Is

Rocky Linux is an enterprise Linux distribution built from Red Hat Enterprise Linux sources. It is fully compatible with RHEL, which means it uses the same package structure, system management approach, SELinux support, and enterprise security concepts. Because of its enterprise focus, it is suitable for real-world Linux administration practice. It gives me a stable, long-term environment for learning domain integration and server security.

Rocky Linux uses systemd, supports SELinux by default, and includes modern tools for authentication such as SSSD and Kerberos integration. These capabilities make it ideal for connecting to Active Directory.
